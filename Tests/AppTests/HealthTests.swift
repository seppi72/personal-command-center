...
struct HealthTests {
    ...
    private func withHealthApp<T>(_ test: (Application) async throws -> T) async throws -> T {
        try await withApp(configure: { app in
            setenv("AUTH_TOKENS", "test-token-one,test-token-two", 1)
    ...
    /// Two separate "clients" (here: two different device tokens) calling
    /// the same running backend must see one shared, advancing count — not
    /// two independent counters — proving there's no per-client local store
    /// to drift out of sync (ADR-0001).
    @Test("two clients calling the same backend see one shared, advancing count")
    func twoClientsShareIdenticalBackendState() async throws {
        try await withHealthApp { app in
            ...
            try await app.testing().test(
                .GET, "/v1/health",
                headers: ["Authorization": "Bearer test-token-two"],
                afterResponse: { res async throws in
                    let body = try res.content.decode(HealthResponse.self)
