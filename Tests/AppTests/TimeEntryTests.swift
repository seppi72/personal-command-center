...
extension AppTestSuite {
    ...
    struct TimeEntryTests {
        ...
        @Test("deleting the Task/Project/Client/Course a Time Entry is attached to deletes the Time Entry")
        func deletingContainerCascadesToTimeEntry() async throws {
            try await withTimeEntriesApp { app in
                let task = try await makeTask(app)
                let taskID = try task.requireID()
                let start = Date(timeIntervalSince1970: 1_800_000_000)
                ...
                let stored = try await TimeEntry.find(entryID, on: app.db)
                #expect(stored == nil)
            }
        }
    }
