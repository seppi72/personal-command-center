...
/// `TaskTests`, `DeadlineTests`, `ClientTests`, and `TimeEntryTests` (which
/// share the `projects`, `tasks`, `clients`, and/or `courses` tables) from
/// running concurrently and corrupting each other's rows.
/// `PersonalCommitmentTests` owns its own tables and doesn't share them with
/// any other suite, but is nested here too for one consistent top-level
/// ordering rather than a special case.
@Suite(.serialized)
struct AppTestSuite {}
